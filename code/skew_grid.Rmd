...
Instead of directly observing the $GF$ at $s$, measurements generally include measurement error such that:
$$ y(s_i) = u(s_i) + e_i $$
...
spde1 <- inla.spde2.pcmatern(
   ...
   alpha   = 2,   # related to smoothness parameter of the process (mesh?) See Nov2017_tutorial, p. 16 
   # 
...
### SPDE Solution on the model area

```{r proj_grid}

xlim    <- seq(-87.1, -80.0, by = 0.1); ylim <- seq(37.5, 44.0, by = 0.1)
loc_mat <- as.matrix(expand.grid(x = xlim, y = ylim))
colnames(loc_mat) <- c('Longitude', 'Latitude')

pgrid0 <- inla.mesh.project(femesh, xlim = c(-87.1, -80.0), ylim = c(37.5, 44.0),
                            dims = c( length(seq(-87.1, -80.0, by = 0.1)),
                                      length(seq( 37.5,  44.0, by = 0.1))))
